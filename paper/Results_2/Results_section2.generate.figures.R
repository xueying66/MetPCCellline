...
pdf('./plot/mutation.profile.MMR.pdf', width = 30, height = 15)
p1+p2
dev.off()
...
load('./data/Prostate Adenocarcinoma.RData')  
load(file = './plot/mutation.burden.RData')
...
# Fig.2d hallmark ----------------------------------------------------
hall <- read.delim(file = './data/MSigDB_Hallmark_2020_table (3).txt') 
...
ggsave('./plot/DE.MSI.non.MSI.ORA.pdf', width = 16, height = 12)
